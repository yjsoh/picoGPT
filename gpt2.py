...
def transformer_block(
    x, mlp, attn, ln_1, ln_2, n_head, kvcache=None
):  # [n_seq, n_embd] -> [n_seq, n_embd]
    # multi-head causal self attention
    attn_out, kvcache_updated = mha(
        layer_norm(x, **ln_1), **attn, n_head=n_head, kvcache=kvcache
    )
...
def gpt2(
    inputs, wte, wpe, blocks, ln_f, n_head, kvcache=None
):  # [n_seq] -> [n_seq, n_vocab]
    if not kvcache:
